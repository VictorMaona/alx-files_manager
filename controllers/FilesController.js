...
const fileQueue = new Queue('fileQueue', 'redis://0.0.0.0:5000');
...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    if (type === 'folder') {
      ...
      // the const storeThis = buff.toString('utf-8');
      try {
        try {
          await fs.mkdir(filePath);
        } catch (error) {
          // pass. file already exists an error is raised
        }
